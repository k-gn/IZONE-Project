...
//		HttpSession session = request.getSession();
		...
		if(dbData != null) {
			if(encoder.matches(inputData.getPassword(), dbData.getPassword())) {
				...
				if(inputData.isAutoLogin()) {
					...
					loginCookie.setPath("/"); // 저장 경로, /~ 로 오는 모든 요청에 적용
					loginCookie.setMaxAge((int)limitTime);
					response.addCookie(loginCookie);
					
					// 자동 로그인 유지시간을 날짜 객체로 변환
					// 현재 시간에서 3개월이 지난 시점의 밀리초를 날짜로 바꿔서 객체로 포장하기
	...
	public ModelAndView logout(HttpSession session, HttpServletRequest request,
								HttpServletResponse response) {
		UserVO user = (UserVO)session.getAttribute("login"); // Object타입이라 downcasting
		...
		if(user != null) {
			...
			// 2. 존재하면 쿠키 수명을 0초로 수정
			// 3. 응답객체를 통해 로컬에 0초짜리 쿠키 재전송 -> 쿠키 삭제
			// 4. 서비스를 통해 db컬럼 레코드 재설정
			// 5. session_id -> "none", limit_time -> 현재시간
