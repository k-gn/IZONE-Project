...
$(function() {
	...
	$('#user_id').on('keyup', function() {
		...
		//ID 중복확인 비동기 처리
			...
			$.ajax({
				type: "POST", // 아이디 정보가 남으면 안되서 post
				url: "/user/checkId",	
				headers: { 
	                "Content-Type": "application/json"
	            },
				dataType: "text",
				data: id, // 단순 문자열, 단일 데이터를 보낼 경우 string 형태로 되있기 때문에 그냥 적어도 된다.
	...
	$('#signup-btn').click(function(e) {
		if(chk1 && chk2 && chk3 && chk4) {
			...
			$.ajax({
				...
				dataType: "text", //응답받을 데이터의 형태 (text : joinSuccess같은거 올 때 사용)
				data: JSON.stringify(user), //서버로 전송할 데이터, 바디에 담아서 보낸다. 
				//(JSON.stringify(user) : user를 json형태로 변환)
